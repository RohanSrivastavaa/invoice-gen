...
// Usage: const { user, error } = await verifySession(request);
 ...
 * Returns { user, isAdmin } on success, throws on failure.
 */
export async function verifySession(request) {
  ...
  // Fetch consultant row to check is_admin
  const { data: consultant } = await supabaseAdmin
    .from("consultants")
    .select("consultant_id, is_admin, email")
    .eq("email", user.email)
    .maybeSingle();
  ...
  return {
    ...
    isAdmin: consultant?.is_admin === true,
  };
}
