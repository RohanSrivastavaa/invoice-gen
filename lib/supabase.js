...
export async function completeOnboarding(email, form) {
  ...
  if (placeholder) {
    ...
    // ✅ Delete the orphaned sign-in row FIRST before updating the placeholder
    // This prevents the unique constraint violation on consultant_id
    await supabase.from("consultants").delete().eq("email", email);

    // Now safely claim the placeholder row
    ...
    // No placeholder — normal flow, just update their own row
    const { error } = await supabase
      .from("consultants")
      .update({
