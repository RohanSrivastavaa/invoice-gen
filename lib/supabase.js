...
export async function fetchConsultant(email, googleName) {
  ...
  // If consultant exists and their name is still the placeholder (email prefix),
  // update it with their real Google display name
  if (data && googleName && data.name === email.split("@")[0]) {
    await supabase
      .from("consultants")
      .update({ name: googleName })
      .eq("email", email);
    return { ...data, name: googleName };
  }
...
export async function completeOnboarding(email, form) {
  ...
  // complete-onboarding verifies the session server-side too
  const sessionToken = await getAccessToken();

  const res = await fetch("/api/complete-onboarding", {
    method: "POST",
    headers: authHeaders(sessionToken),
    body: JSON.stringify({ email, consultantId, pan, gstin, bankBeneficiary, bankName, bankAccount, bankIfsc, name }),
  });
